...
public class QuercusClass extends NullValue {
    ...
    public QuercusClass(ModuleContext moduleContext, ClassDef classDef, QuercusClass parent){
        ...
        for (int i = 0; i < classDefList.length; i++) {
            if (classDefList[i] instanceof JavaClassDef) javaClassDef = (JavaClassDef) classDefList[i];
        }
        ...
        for (int i = classDefList.length - 1; i >= 0; i--) {
            ...
            if (classDef == null) {
                throw new NullPointerException("classDef:" + _classDef + " i:" + i + " parent:" + parent);
            }
        ...
        if (_destructor == null && parent != null) _destructor = parent.getDestructor();
    }

    private void addInstances(HashSet<String> instanceofSet, HashSet<String> ifaceSet, ClassDef classDef) {
        ...
        for (String iface : classDef.getInterfaces()) {
            ...
            cls = Env.getInstance().findClass(iface, !isJavaClassDef, true);

            if (cls == null) {
                throw new QuercusRuntimeException(L.l("cannot find interface {0}", iface));
            }
    ...
    private void addTraitMethods(HashSet<String> traitSet, ClassDef classDef) {
        for (String trait : classDef.getTraits()) {
            ...
            if (cls == null) {
                Env.getInstance().createErrorException(L.l("cannot find trait {0}", trait));
            }
    ...
    public boolean isModified() {
        ...
        else if (_parent != null) return _parent.isModified();
        else return false;
    }
    ...
    public void setModified() {
        if (!_isModified) {
            ...
            if (_cacheRef != null) {
                ...
                if (cacheClass != null) cacheClass.setModified();
            }
        }
    }
    ...
    public void setArrayDelegate(ArrayDelegate delegate) {
        if (log.isLoggable(Level.FINEST)) log.log(Level.FINEST, L.l("{0} adding array delegate {1}", this, delegate));
    ...
    public void setTraversableDelegate(TraversableDelegate delegate) {
        if (log.isLoggable(Level.FINEST)) log.log(Level.FINEST,
                                                  L.l("{0} setting traversable delegate {1}", this, delegate));
    ...
    public void setCountDelegate(CountDelegate delegate) {
        if (log.isLoggable(Level.FINEST)) log.log(Level.FINEST, L.l("{0} setting count delegate {1}", this, delegate));
     ...
     * Adds a field.
     */
    public void addField(String declaringClassName, StringValue name, Expr initExpr, FieldVisibility visibility) {
        ClassField field = new ClassField(declaringClassName, name, initExpr, visibility);
     ...
     * Adds a trait field.
     */
    public void addTraitField(String declaringClassName, StringValue name, Expr initExpr, FieldVisibility visibility) {
        ClassField existingField = _fieldMap.get(name);

        if (existingField != null && !existingField.getInitValue().equals(initExpr)) {
            Env env = Env.getInstance();
            throw env.createErrorException(L.l("trait field {0}->{1} conflicts with class field {2}->{1}.",
                                               declaringClassName, name, getName()));
        }

        boolean isTraitField = true;
        ClassField field = new ClassField(declaringClassName, name, initExpr, visibility, isTraitField);
    ...
    public void addMethod(StringValue name, AbstractFunction fun) {
        ...
        if (existingFun == null || !fun.isAbstract()) {
            ...
            Env.getInstance().error(L.l("cannot make non-abstract function {0}:{1}() abstract", getName(), name));
        }
    }
     ...
     */
    public void addTraitMethod(String bindingClassName, StringValue name, AbstractFunction fun) {
    ...
    private void addTraitMethod(String bindingClassName, StringValue name, StringValue alias, AbstractFunction fun) {
        if (fun == null) {
            throw new NullPointerException(L.l("'{0}' is a null function", name));
        }
        ...
        if (existingFun == null || existingFun.isAbstract()) {
            ...
            if (insteadofMap == null) {
                traitCollisionError(getName(), name, fun.getDeclaringClassName(), existingFun.getDeclaringClassName());
            }

            int result = insteadofMap.get(name, fun.getDeclaringClassName(), existingFun.getDeclaringClassName());
    ...
    private static final void traitCollisionError(String className, StringValue funName, String traitName,
                                                  String existingTraitName) {
        Env.getInstance().error(L.l("cannot add trait method {0}::{1}() to class {2}"
                                            + " because it collides with trait method {3}::{1}() ", traitName, funName,
     ...
     * Adds a static class field.
     */
    public void addStaticTraitFieldExpr(String bindingClassName, StringValue name, Expr value) {
    ...
    public void validate(Env env) {
        if (!isAbstract() && !isInterface() && !isTrait()) {
            for (AbstractFunction fun : _methodMap.values()) {
                /*
                 * XXX: abstract methods need to be validated php/393g, php/393i, php/39j2 if (! (absFun instanceof
                 * Function)) continue; Function fun = (Function) absFun;
                 */

                boolean isAbstract;

                // php/093g constructor
                if (_constructor != null && fun.getName().equals(_constructor.getName())) isAbstract = _constructor.isAbstract();
                else isAbstract = fun.isAbstract();
    ...
    public void init(Env env) {
        if (_staticFieldExprMap.size() == 0) return;
    ...
    public Value getStaticFieldValue(Env env, StringValue name) {
        ...
        if (staticName == null) {
            env.error(L.l("{0}::${1} is an undeclared static field", _className, name));
    ...
    public Value setStaticFieldRef(Env env, StringValue name, Value value) {
        ...
        if (staticName == null) {
            env.error(L.l("{0}::{1} is an unknown static field", _className, name));
    ...
    public Value getStaticField(Env env, StringValue name) {
        ...
        if (staticName != null) return env.getStaticValue(staticName);
        else return null;
    }
     ...
     */
    /*
     * public Value callNew(Env env, Expr []args) { Value object = _classDef.callNew(env, args); if (object != null)
     * return object; object = newInstance(env); AbstractFunction fun = findConstructor(); if (fun != null) {
     * fun.callMethod(env, object, args); } return object; }
     */
    ...
    public Value createObject(Env env) {
        if (isAbstract()) {
            throw env.createErrorException(L.l("abstract class '{0}' cannot be instantiated.", _className));
        } else if (isInterface()) {
            throw env.createErrorException(L.l("interface '{0}' cannot be instantiated.", _className));
        } else if (isTrait()) {
            throw env.createErrorException(L.l("trait '{0}' cannot be instantiated.", _className));
        }
    ...
    public Value callNew(Env env, Object parentJavaObject, Value... args) {
        ...
        try {
            ...
            ObjectValue objectValue = new ObjectExtJavaValue(this, parentJavaObject, _javaClassDef);
    ...
    public Value callNew(Env env, Value... args) {
        ...
        try {
            ...
            if (fun != null) fun.callNew(env, this, objectValue, args);
            else {
                // if expr
            }
    ...
    /**
     * Puts the interfaces that this class and its parents implements into the array.
     */
    private void getInterfaces(Env env, ArrayValue array, boolean autoload, boolean isTop) {
        ...
        if (_parent != null) _parent.getInterfaces(env, array, autoload, false);
    }
    ...
    public boolean implementsInterface(Env env, String name) {
        ...
        for (int i = 0; i < defList.length; i++) {
            ...
            if (def.isInterface() && def.getName().equals(name)) return true;
            ...
            for (int j = 0; j < defNames.length; j++) {
                ...
                if (cls.implementsInterface(env, name)) return true;
            }
        }

        if (_parent != null) return _parent.implementsInterface(env, name);
        else return false;
    }
    ...
    public Value getField(Env env, Value qThis, StringValue name) {
        ...
        if (_fieldGet != null) {
            if (!env.pushFieldGet(Env.OVERLOADING_TYPES.FIELDGET, qThis.getClassName(), name)) return UnsetValue.UNSET;
        ...
        } else return UnsetValue.UNSET;
    }

    /**
     * Implements the __isset method call. __isset() is triggered by calling isset() or empty() on inaccessible
     * properties.
     */
    public boolean issetField(Env env, Value qThis, StringValue name) {
        ...
        if (_isset != null) {
            if (!env.pushFieldGet(Env.OVERLOADING_TYPES.ISSET, qThis.getClassName(), name)) return false;
    ...
    public void unsetField(StringValue name) {
        if (_fieldMap.containsKey(name)) _fieldMap.remove(name);
    }
    ...
    public Value unsetField(Env env, Value qThis, StringValue name) {
        ...
        if (_unset != null) {
            if (!env.pushFieldGet(Env.OVERLOADING_TYPES.UNSET, qThis.getClassName(), name)) return UnsetValue.UNSET;
    ...
    public Value setField(Env env, Value qThis, StringValue name, Value value) {
        if (_fieldSet != null) {
            if (!env.pushFieldGet(Env.OVERLOADING_TYPES.FIELDSET, qThis.getClassName(), name)) return UnsetValue.UNSET;
    ...
    public Value callConstructor(Env env, Value qThis, Value... args) {
        ...
        if (cons == null) {
            ...
            env.error(L.l("{0}::{1}() cannot be called statically", getName(), cons.getName()));
        }
    ...
    public final Value callMethod(Env env, Value qThis, StringValue methodName, Value[] args) {
        return callMethod(env, qThis, methodName, methodName.hashCodeCaseInsensitive(), args);
    }
    ...
    public final Value callMethod(Env env, Value qThis, StringValue methodName) {
        return callMethod(env, qThis, methodName, methodName.hashCodeCaseInsensitive());
    }
    ...
    public final Value callMethod(Env env, Value qThis, StringValue methodName, Value a1) {
        return callMethod(env, qThis, methodName, methodName.hashCodeCaseInsensitive(), a1);
    }
    ...
    public final Value callMethod(Env env, Value qThis, StringValue methodName, Value a1, Value a2) {
        return callMethod(env, qThis, methodName, methodName.hashCodeCaseInsensitive(), a1, a2);
    }
    ...
    public final Value callMethod(Env env, Value qThis, StringValue methodName, Value a1, Value a2, Value a3) {
        return callMethod(env, qThis, methodName, methodName.hashCodeCaseInsensitive(), a1, a2, a3);
    }
    ...
    public final Value callMethod(Env env, Value qThis, StringValue methodName, Value a1, Value a2, Value a3, Value a4) {
        return callMethod(env, qThis, methodName, methodName.hashCodeCaseInsensitive(), a1, a2, a3, a4);
    }
    ...
    public final Value callMethod(Env env, Value qThis, StringValue methodName, Value a1, Value a2, Value a3, Value a4,
                                  Value a5) {
        return callMethod(env, qThis, methodName, methodName.hashCodeCaseInsensitive(), a1, a2, a3, a4, a5);
    }
    ...
    public final Value callMethodRef(Env env, Value qThis, StringValue methodName, Value[] args) {
        return callMethodRef(env, qThis, methodName, methodName.hashCodeCaseInsensitive(), args);
    }
    ...
    public final Value callMethodRef(Env env, Value qThis, StringValue methodName) {
        return callMethodRef(env, qThis, methodName, methodName.hashCodeCaseInsensitive());
    }
    ...
    public final Value callMethodRef(Env env, Value qThis, StringValue methodName, Value a1) {
        return callMethodRef(env, qThis, methodName, methodName.hashCodeCaseInsensitive(), a1);
    }
    ...
    public final Value callMethodRef(Env env, Value qThis, StringValue methodName, Value a1, Value a2) {
        return callMethodRef(env, qThis, methodName, methodName.hashCodeCaseInsensitive(), a1, a2);
    }
    ...
    public final Value callMethodRef(Env env, Value qThis, StringValue methodName, Value a1, Value a2, Value a3) {
        return callMethodRef(env, qThis, methodName, methodName.hashCodeCaseInsensitive(), a1, a2, a3);
    }
    ...
    public final Value callMethodRef(Env env, Value qThis, StringValue methodName, Value a1, Value a2, Value a3,
                                     Value a4) {
        return callMethodRef(env, qThis, methodName, methodName.hashCodeCaseInsensitive(), a1, a2, a3, a4);
    }
    ...
    public final Value callMethodRef(Env env, Value qThis, StringValue methodName, Value a1, Value a2, Value a3,
                                     Value a4, Value a5) {
        return callMethodRef(env, qThis, methodName, methodName.hashCodeCaseInsensitive(), a1, a2, a3, a4, a5);
    }
     ...
     */
    /*
     * private Value callStaticMethod(Env env, Value thisValue, StringValue methodName, Expr []args) { QuercusClass
     * oldClass = env.setCallingClass(this); try { return callMethod(env, thisValue, methodName, args); } finally {
    ...
    public Value callMethod(Env env, StringValue methodName, int hash, Value a1) {
        return callMethod(env, this, methodName, hash, a1);
    }
    ...
    public Value callMethod(Env env, StringValue methodName, int hash, Value a1, Value a2) {
        return callMethod(env, this, methodName, hash, a1, a2);
    }
    ...
    public Value callMethod(Env env, StringValue methodName, int hash, Value a1, Value a2, Value a3) {
        return callMethod(env, this, methodName, hash, a1, a2, a3);
    }
    ...
    public Value callMethod(Env env, StringValue methodName, int hash, Value a1, Value a2, Value a3, Value a4) {
        return callMethod(env, this, methodName, hash, a1, a2, a3, a4);
    }
    ...
    public Value callMethod(Env env, StringValue methodName, int hash, Value a1, Value a2, Value a3, Value a4, Value a5) {
        return callMethod(env, this, methodName, hash, a1, a2, a3, a4, a5);
    }
    ...
    public Value callMethodRef(Env env, StringValue methodName, int hash, Value a1) {
        return callMethodRef(env, this, methodName, hash, a1);
    }
    ...
    public Value callMethodRef(Env env, StringValue methodName, int hash, Value a1, Value a2) {
        return callMethodRef(env, this, methodName, hash, a1, a2);
    }
    ...
    public Value callMethodRef(Env env, StringValue methodName, int hash, Value a1, Value a2, Value a3) {
        return callMethodRef(env, this, methodName, hash, a1, a2, a3);
    }
    ...
    public Value callMethodRef(Env env, StringValue methodName, int hash, Value a1, Value a2, Value a3, Value a4) {
        return callMethodRef(env, this, methodName, hash, a1, a2, a3, a4);
    }
    ...
    public Value callMethodRef(Env env, StringValue methodName, int hash, Value a1, Value a2, Value a3, Value a4,
                               Value a5) {
        return callMethodRef(env, this, methodName, hash, a1, a2, a3, a4, a5);
    }
    ...
    public final AbstractFunction getStaticFunction(String name) {
        ...
        /*
         * if (fun != null) return fun; fun = findStaticFunctionLowerCase(name.toLowerCase(Locale.ENGLISH));
         */

        if (fun != null) return fun;
        else {
            throw new QuercusRuntimeException(L.l("{0}::{1} is an unknown method", getName(), name));
        }
    }
    ...
    public final Value getConstant(Env env, StringValue name) {
        ...
        if (expr != null) return expr.eval(env);

        Object obj = _constJavaMap.get(name);

        if (obj != null) return env.wrapJava(obj);

        throw new QuercusRuntimeException(L.l("{0}::{1} is an unknown constant", getName(), name));
    }
    ...
    public final boolean hasConstant(StringValue name) {
        if (_constMap.get(name) != null) return true;
        else return _constJavaMap.get(name) != null;
    }
    ...
    public boolean equals(Object o) {
        ...
        else if (!(o instanceof QuercusClass)) return false;

        QuercusClass qClass = (QuercusClass) o;

        if (_classDef != qClass._classDef) return false;

        if (_javaClassDef != qClass._javaClassDef) return false;

        if (_parent == qClass._parent) return true;

        else return (_parent != null && _parent.equals(qClass._parent));
    }
