...
public class UrlModule extends AbstractQuercusModule {
    ...
    public static String base64_encode(StringValue s) {
        ...
        if (offset < strlen) buffer[0] = (byte) s.charAt(offset);
        if (offset + 1 < strlen) buffer[1] = (byte) s.charAt(offset + 1);
        if (offset + 2 < strlen) buffer[2] = (byte) s.charAt(offset + 2);
    ...
    public static Value get_headers(Env env, String urlString, @Optional Value format) {
        ...
        try {
            ...
            if (url.getPath() != null) request.append(url.getPath());

            if (url.getQuery() != null) request.append("?" + url.getQuery());

            if (url.getRef() != null) request.append("#" + url.getRef());
            ...
            if (format.toBoolean()) {
                for (String line = reader.readLine(); line != null; line = reader.readLine()) {
                    ...
                    if (colon < 0) result.put(env.createString(line.trim()));
                    else {
                        StringValue key = env.createString(line.substring(0, colon).trim());

                        StringValue value;

                        if (colon < line.length()) value = env.createString(line.substring(colon + 1).trim());
                        else value = env.getEmptyString();

                        if (result.get(key) != UnsetValue.UNSET) values = (ArrayValue) result.get(key);
                        else {
                ...
                for (Value key : result.keySet()) {
                    ...
                    if (value.isArray() && ((ArrayValue) value).getSize() == 1) result.put(key,
                                                                                           ((ArrayValue) value).get(LongValue.ZERO));
                }
            } else {
                for (String line = reader.readLine(); line != null; line = reader.readLine()) {
                    line = line.trim();
    ...
    public static Value get_meta_tags(Env env, StringValue filename, @Optional boolean useIncludePath) {
        ...
        try {
            ...
            if (stream == null || !(stream instanceof BinaryInput)) return result;
            ...
            while (!input.isEOF()) {
                ...
                if (tag.equalsIgnoreCase("meta")) {
                    ...
                    while ((attr = getNextAttribute(input)) != null) {
                        if (name == null && attr[0].equalsIgnoreCase("name")) {
                            if (attr.length > 1) name = attr[1];
                        } else if (content == null && attr[0].equalsIgnoreCase("content")) {
                            if (attr.length > 1) content = attr[1];
                        }
                ...
                } else if (tag.equalsIgnoreCase("/head")) break;
            }
        } catch (IOException e) {
            env.warning(e);
        } finally {
    ...
    public static Value http_build_query(Env env, Value formdata, @Optional StringValue numeric_prefix,
                                         @Optional("'&'") StringValue separator) {
        StringValue result = env.createUnicodeBuilder();

        httpBuildQueryImpl(env, result, formdata, env.getEmptyString(), numeric_prefix, separator);

        return result;
    }

    private static void httpBuildQueryImpl(Env env, StringValue result, Value formdata, StringValue path,
                                           StringValue numeric_prefix, StringValue separator) {
     ...
     * @Optional String prefix) { StringBuilder sb = new StringBuilder(); int index = 0; if (value instanceof
     * ArrayValue) { ArrayValue array = (ArrayValue) value; for (Map.Entry<Value,Value> entry : array.entrySet()) {
     * Value keyValue = entry.getKey(); Value v = entry.getValue(); String key; if (keyValue.isLongConvertible()) key =
     * prefix + keyValue; else key = keyValue.toString(); if (v instanceof ArrayValue) http_build_query(sb, key,
     * (ArrayValue) v); else { if (sb.length() > 0) sb.append('&'); sb.append(key); sb.append('='); urlencode(sb,
     * v.toString()); } } } return sb.toString(); }
     */
     ...
     */
    /*
     * private void http_build_query(StringBuilder sb, String prefix, ArrayValue array) { for (Map.Entry<Value,Value>
     * entry : array.entrySet()) { Value keyValue = entry.getKey(); Value v = entry.getValue(); String key = prefix +
     * '[' + keyValue + ']'; if (v instanceof ArrayValue) http_build_query(sb, key, (ArrayValue) v); else { if
     * (sb.length() > 0) sb.append('&'); sb.append(key); sb.append('='); urlencode(sb, v.toString()); } } }
     */

    /**
     * Parses the URL into an array.
     */
    public static Value parse_url(Env env, StringValue str, @Optional("-1") int component) {
    ...
    private static void parseUrl(Env env, StringValue str, ArrayValue array, boolean isUnicode) {
        ...
        if (0 <= i && hasHost) {
            ...
            if (i < colon) {
                ...
                int end;
                if (i < slash) end = slash;
                else if (i < question) end = question + 1;
                else if (i < pound) end = pound + 1;
                else end = strlen;
                ...
                if (0 < end - (colon + 1)) {
                    ...
                    for (int j = colon + 1; j < end; j++) {
                        ...
                        if ('0' <= ch && ch <= '9') port = port * 10 + ch - '0';
                        else break;
                    }
        ...
        if (user != null) array.put(USER_V, USER_U, user, isUnicode);

        if (pass != null) array.put(PASS_V, PASS_U, pass, isUnicode);
        ...
        if (0 <= pound) {
            if (i < pound) {
                ...
                if (0 <= question) array.put(QUERY_V, QUERY_U, sb, isUnicode);
                else array.put(PATH_V, PATH_U, sb, isUnicode);
            }
    ...
    private static void urlencode(StringValue sb, StringValue str) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            else if ('0' <= ch && ch <= '9') sb.append(ch);
    ...
    /**
     * Finds the next attribute in the stream and return the key and value as an array.
     */
    private static String[] getNextAttribute(BinaryInput input) throws IOException {
        ...
        while (!input.isEOF()) {
            ...
            if (isValidAttributeCharacter(ch)) attribute.append((char) ch);
            else {
                input.unread();
                break;
            }
        ...
        if (input.isEOF()) return new String[] { attribute.toString() };
        ...
        while (!input.isEOF()) {
            ...
            // mimics PHP behavior
            if ((quoted && ch == quote) || (!quoted && Character.isWhitespace(ch)) || ch == '>') break;
    ...
    private static void consumeWhiteSpace(BinaryInput input) throws IOException {
        ...
        if (!Character.isWhitespace(ch)) input.unread();
    }
