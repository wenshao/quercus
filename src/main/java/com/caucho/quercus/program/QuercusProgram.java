...
public class QuercusProgram {
    ...
    private HashMap<StringValue, Function>       _functionMapLowerCase = new HashMap<StringValue, Function>();
     ...
     */
    public QuercusProgram(QuercusContext quercus, Path sourceFile, HashMap<StringValue, Function> functionMap,
                          ArrayList<Function> functionList, HashMap<String, InterpretedClassDef> classMap,
                          ArrayList<InterpretedClassDef> classList, FunctionInfo functionInfo, Statement statement){
        _quercus = quercus;
        ...
        if (sourceFile != null) addDepend(sourceFile);

        _functionMap = functionMap;
        _functionList = functionList;

        for (Map.Entry<StringValue, Function> entry : functionMap.entrySet()) {
            _functionMapLowerCase.put(entry.getKey().toLowerCase(Locale.ENGLISH), entry.getValue());
        }
    ...
    public AbstractFunction findFunction(StringValue name) {
        ...
        if (!_quercus.isStrict()) fun = _functionMapLowerCase.get(name.toLowerCase(Locale.ENGLISH));
    ...
    public void init(Env env) {
        /*
         * for (Map.Entry<String,InterpretedClassDef> entry : _classMap.entrySet()) { entry.getValue().init(env); }
         */
    }
    ...
    public void importDefinitions(Env env) {
        for (Map.Entry<StringValue, Function> entry : _functionMap.entrySet()) {
            ...
            if (fun.isGlobal()) env.addFunction(entry.getKey(), fun);
        }
    ...
    class PageDependency implements Dependency {
        ...
        public boolean isModified() {
            if (_compiledPage != null) return _compiledPage.isModified();
            else return _depend.isModified();
        }
