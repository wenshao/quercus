...
public class QuercusContext {
    ...
    private static LruCache<String, UnicodeBuilderValue> _unicodeMap                 = new LruCache<String, UnicodeBuilderValue>(
                                                                                                                                 8 * 1024);

    private static LruCache<String, ConstStringValue>    _stringMap                  = new LruCache<String, ConstStringValue>(
                                                                                                                              8 * 1024);

    private HashMap<String, ModuleInfo>                  _modules                    = new HashMap<String, ModuleInfo>();

    private ArrayList<ModuleInfo>                        _moduleInitList             = new ArrayList<ModuleInfo>();
    ...
    private HashMap<StringValue, AbstractFunction>       _funMap                     = new HashMap<StringValue, AbstractFunction>();

    private HashMap<StringValue, AbstractFunction>       _lowerFunMap                = new HashMap<StringValue, AbstractFunction>();
    ...
    private HashMap<String, Class<?>>                    _javaInitClassMap           = new HashMap<String, Class<?>>();
    ...
    private LruCache<StringValue, QuercusProgram>        _evalCache                  = new LruCache<StringValue, QuercusProgram>(
                                                                                                                                 4096);
     ...
     */
    public Path getPwd() {
        if (_pwd == null) _pwd = new FilePath(System.getProperty("user.dir"));
    ...
    public Path getWorkDir() {
        if (_workDir == null) _workDir = getWebInfDir().lookup("work");
    ...
    public String getScriptEncoding() {
        if (_scriptEncoding != null) return _scriptEncoding;
        else if (isUnicodeSemantics()) return "utf-8";
        else return "utf-8";
    }
    ...
    public String getOutputEncoding() {
        ...
        if (encoding == null) encoding = QuercusContext.INI_UNICODE_FALLBACK_ENCODING.getAsString(this);
    ...
    private void initJavaClasses() {
        for (Map.Entry<String, Class<?>> entry : _javaInitClassMap.entrySet()) {
            ...
            try {
                if (cls.isAnnotationPresent(ClassImplementation.class)) _moduleContext.introspectJavaImplClass(name,
                                                                                                               cls,
                                                                                                               null);
                else _moduleContext.introspectJavaClass(name, cls, null, null);
            } catch (Exception e) {
    ...
    public void setServerEnv(String name, String value) {
        // php/3j58
        if (isUnicodeSemantics()) setServerEnv(createUnicodeString(name), createUnicodeString(value));
        else setServerEnv(createString(name), createString(value));
    }
     ...
     * _defCache.get(key); if (defStateRef != null) { DefinitionState defState = defStateRef.get(); if (defState !=
     * null) { _defCacheHitCount++; return defState.copyLazy(); } } _defCacheMissCount++; return null; }
     */
    ...
    protected void extendFunctionMap(StringValue name, int id) {
        if (_functionMap.length <= id) {
            ...
            System.arraycopy(_functionMap, 0, functionMap, 0, _functionMap.length);
    ...
    public int getConstantId(String name) {
        // php/3j12
        if (isUnicodeSemantics()) return getConstantId(new UnicodeBuilderValue(name));
        else return getConstantId(new ConstStringValue(name));
    }
    ...
    public QuercusModule findModule(String name) {
        ...
        if (moduleInfo != null) module = moduleInfo.getModule();
        else module = getModuleContext().findModule(name);

        if (module == null) throw new IllegalStateException(L.l("'{0}' is an unknown quercus module", name));
    ...
    public boolean isExtensionLoaded(String name) {
        return _extensionSet.contains(name) || _extensionSetLowerCase.contains(name.toLowerCase(Locale.ENGLISH));
    }
    ...
    public void init() {
        ...
        for (Map.Entry<String, String> entry : System.getenv().entrySet()) {
            _serverEnvMap.put(createString(entry.getKey()), createString(entry.getValue()));
        }
        ...
        _includeCache = new TimedCache<IncludeKey, Path>(getIncludeCacheMax(), getIncludeCacheTimeout());
    ...
    private void initModuleInfo(ModuleInfo info, HashSet<String> disableSet) {
        ...
        if (info.getModule() instanceof ModuleStartupListener) _moduleStartupListeners.add((ModuleStartupListener) info.getModule());
        ...
        if (isUnicodeSemantics()) map = info.getUnicodeConstMap();
        else map = info.getConstMap();
        ...
        for (Map.Entry<String, Method[]> entry : info.getFunctions().entrySet()) {
            ...
            AbstractJavaMethod fun = _moduleContext.createStaticFunction(info.getModule(), methods[0]);

            for (int i = 1; i < methods.length; i++) {
                AbstractJavaMethod overload = _moduleContext.createStaticFunction(info.getModule(), methods[i]);
    ...
    public static Value objectToValue(Object obj) {
        ...
        else if (Byte.class.equals(obj.getClass()) || Short.class.equals(obj.getClass())
                 || Integer.class.equals(obj.getClass()) || Long.class.equals(obj.getClass())) {
    ...
    protected void initLocal() {
        ...
        String[] paths = new String[] { "/usr/share/php", "/usr/lib/php", "/usr/local/lib/php", "/usr/share/pear",
    ...
    static class IncludeKey {
        ...
        public int hashCode() {
            ...
            if (_pwd != null) hash = 65537 * hash + _pwd.hashCode();
        ...
        public boolean equals(Object o) {
            ...
            return (_include.equals(key._include) && _includePath.equals(key._includePath) && _pwd.equals(key._pwd) && _scriptPwd.equals(key._scriptPwd));
        }
    }
    ...
    static {
        ...
        /*
         * String includePath; if (Path.isWindows()) includePath = "." + FileModule.PATH_SEPARATOR + "C:\\php5\\pear";
         * else includePath = "." + FileModule.PATH_SEPARATOR + "/usr/share/php" + FileModule.PATH_SEPARATOR +
         * "/usr/share/pear"; INI_INCLUDE_PATH = _ini.add( "include_path", includePath, IniDefinition.PHP_INI_ALL);
         */
    }

    public static final IniDefinition INI_INCLUDE_PATH                  = _ini.add("include_path", ".",
                                                                                   IniDefinition.PHP_INI_ALL);
    public static final IniDefinition INI_REGISTER_LONG_ARRAYS          = _ini.add("register_long_arrays", true,
                                                                                   IniDefinition.PHP_INI_PERDIR);
    public static final IniDefinition INI_ALWAYS_POPULATE_RAW_POST_DATA = _ini.add("always_populate_raw_post_data",
                                                                                   false, IniDefinition.PHP_INI_PERDIR);

    // unicode ini
    public static final IniDefinition INI_UNICODE_SEMANTICS             = _ini.add("unicode.semantics", false,
                                                                                   IniDefinition.PHP_INI_SYSTEM);
    public static final IniDefinition INI_UNICODE_FALLBACK_ENCODING     = _ini.add("unicode.fallback_encoding",
                                                                                   "utf-8", IniDefinition.PHP_INI_ALL);
    public static final IniDefinition INI_UNICODE_FROM_ERROR_MODE       = _ini.add("unicode.from_error_mode", "2",
                                                                                   IniDefinition.PHP_INI_ALL);
    public static final IniDefinition INI_UNICODE_FROM_ERROR_SUBST_CHAR = _ini.add("unicode.from_error_subst_char",
                                                                                   "3f", IniDefinition.PHP_INI_ALL);
    public static final IniDefinition INI_UNICODE_HTTP_INPUT_ENCODING   = _ini.add("unicode.http_input_encoding", null,
                                                                                   IniDefinition.PHP_INI_ALL);
    public static final IniDefinition INI_UNICODE_OUTPUT_ENCODING       = _ini.add("unicode.output_encoding", null,
                                                                                   IniDefinition.PHP_INI_ALL);
    public static final IniDefinition INI_UNICODE_RUNTIME_ENCODING      = _ini.add("unicode.runtime_encoding", null,
                                                                                   IniDefinition.PHP_INI_ALL);
    public static final IniDefinition INI_UNICODE_SCRIPT_ENCODING       = _ini.add("unicode.script_encoding", null,
                                                                                   IniDefinition.PHP_INI_ALL);
}
