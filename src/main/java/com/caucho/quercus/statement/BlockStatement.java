...
public class BlockStatement extends Statement {
    ...
    public BlockStatement(Location location, ArrayList<Statement> statementList){
        ...
        this.statements = new Statement[statementList.size()];
        statementList.toArray(this.statements);
    ...
    public BlockStatement append(ArrayList<Statement> statementList) {
        Statement[] statements = new Statement[this.statements.length + statementList.size()];

        System.arraycopy(this.statements, 0, statements, 0, this.statements.length);

        for (int i = 0; i < statementList.size(); i++)
            statements[i + this.statements.length] = statementList.get(i);
    ...
    public int fallThrough() {
        for (int i = 0; i < getStatements().length; i++) {
            ...
            if (fallThrough != FALL_THROUGH) return fallThrough;
        }
