...
public class InternalCompilerTools extends AbstractJavaCompiler {
    ...
    private static boolean            _hasCompiler;                                                 // already tested
                                                                                                     // for compiler
    ...
    private void executeInt(String[] path, LineMap lineMap) throws JavaCompileException, IOException {
        ...
        try {
            ...
            if (args != null) argList.addAll(args);
            ...
            if (log.isLoggable(Level.FINER)) {
                ...
                for (int i = 0; i < argList.size(); i++) {
                    if (argList.get(i).equals("-classpath") && !log.isLoggable(Level.FINEST)) {
                        i++;
                        continue;
                    }
            ...
            try {
                ...
                if (javac == null) {
                    ...
                    if (iter.hasNext()) javac = iter.next();
                }

                if (javac == null) throw new ConfigException(
                                                             L.l("javac compiler is not available in {0}. Check that you are using the JDK, not the JRE.",
            ...
            if (errors != null) errors = errors.trim();
            ...
            if (status != 0) throw new JavaCompileException(errors);
        } finally {
            tempStream.destroy();
        }
    }
